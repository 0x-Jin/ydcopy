...
class DamageController extends Controller {
    ...
    public function postnew( Request $req ) {
        ...
        if ( !empty( $reqData[ 'reason' ] ) || !empty( $reqData[ 'person' ] ) || !empty( $reqData[ 'bn' ] ) ) {
            ...
            $detail_id = DB::table( 'damageitem' )->select( 'damage_id' )->distinct( 'damage_id' )
                            ...
                            ->where( function($query) use($reqData) {
                                if ( !empty( $reqData[ 'bn' ] ) ) {//状态
                                    $query->where( 'spec_bn', $reqData[ 'bn' ] );
                                }
                            } )
            ...
            $data_src = DB::table( 'damage' )->select( \DB::raw( "{$select_cols}" ) )
                    ...
                    ->where(function($query) use($reqData){
                        if ( !empty( $reqData[ 'shipment_id' ] ) ) {
                            $query->where( 'shipment_id', $reqData[ 'shipment_id' ] );
                        }
                    })
                    ->where(function($query) use($reqData){
                        if ( !empty( $reqData[ 'mathStart' ] ) ) {
                            $query->where( 'opeator_time','>', strtotime($reqData[ 'mathStart' ]) );
                        }
                    })
                    ->where(function($query) use($reqData){
                        if ( !empty( $reqData[ 'mathEnd' ] ) ) {
                            $query->where( 'opeator_time','<', strtotime($reqData[ 'mathEnd' ]) );
                        }
                    })
                    ->whereIn( 'damage_id', $damage_array )->orderBy( 'opeator_time', 'desc' );
        } else {
            $data_src = DB::table( 'damage' )->select( \DB::raw( "{$select_cols}" ) )
                    ->where( function($query) use($reqData) {
                        if ( !empty( $reqData[ 'platform' ] ) ) {
    ...
    public function doadd( Request $req ) {
        ...
        if($reqData['assumed_ratio'] != 0 && $reqData['assumed_ratio'] != 1) exit( json_encode( array( 'status' => 0, 'data' => '请选择承担比例' ) ) );
        ...
        exit( json_encode( array( 'status' => 1, 'data' => '添加成功' ) ) );
    }
    ...
    public function println( Request $req ) {
        ...
        $damage_main = DB::table( 'damage' )->select( "*" )->where( 'damage_id', $id )->get();
        $damage_main[0]->opeator_time = date('Y-m-d',$damage_main[0]->opeator_time);        
        $damage_detail = DB::table( 'damageitem' )->select( '*' )->where( 'damage_id', $id )->get();
     ...
     //乱码问题
     public function exportall(Request $req) {
        ob_end_clean();
        $cellData = [['店铺名称', '配货单号', '快递单号', '快递名称', '顾客名称', '电话号码', '地址', '备注', '承担比例', '承担总额','操作时间','操作人','交易号','是否货到付款单号','货到付款单号','快递公司','配货状态','商品名称','商品编码','规格编码','数量','销售价','有无返回','原因','责任方']];
        $sql = "select d.shopname,d.shipment_id,d.delivery_id,d.delivery_company,d.customer,d.tellphone,d.address,d.comment,d.assumed_ratio,d.assumed_sum,d.opeator_time,d.opeator,d.trade_id,d.is_cod,d.cod_num,d.deliverycompany,d.shipment_status,i.good_name,i.good_bn,i.spec_bn,i.number,i.sale_price,i.is_return,i.reason,i.assumed_person from damage d left join damageitem i on d.damage_id = i.damage_id";
        $sql .= " where 1";
        if($req->platform)  $sql.= " and d.shopname = '{$req->platform}'";
        if($req->reason) $sql.= " and i.reason = '{$req->reason}'";
        if($req->person) $sql.= " and i.assumed_person = '{$req->person}'";
        if($req->bn) $sql.= " and i.good_bn = '{$req->bn}'";
        if($req->shipment_id) $sql.= " and d.shipment_id = '{$req->shipment_id}'";
        if($req->mathStart) $sql.= " and d.opeator_time >= '".strtotime($req->mathStart)."'";
        if($req->mathEnd) $sql.= " and d.opeator_time <= '".strtotime($req->mathEnd)."'";
        $data = DB::select($sql);
        foreach ($data as $k=>$item) {
            unset($data[$k]->damage_id);
            unset($data[$k]->modify_time);
            $item->opeator_time = date('Y-m-d H:i:s',$item->opeator_time);
            $item->assumed_ratio = $this->to_map['assumed_ratio'][$item->assumed_ratio];
            $item->is_cod = $this->to_map['is_cod'][$item->is_cod];
            $item->reason = $this->to_map['reason'][$item->reason];
            $item->assumed_person = $this->to_map['person'][$item->assumed_person];
            $item = $this->object_to_array($item);
            array_push($cellData, $item);
        }
        $filename = date('Y-m-d H:i:s');
        $excel = Excel::create($filename, function($excel) use ($cellData) {
