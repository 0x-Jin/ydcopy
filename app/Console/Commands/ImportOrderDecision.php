...
class ImportOrderDecision extends Command
{
    ...
    public function handle(Decision $decision){
        ...
        $date = date('h');
        if($date > 4){
            $start = date('Y-m-d').' 04:00:00.000';
            $end = date('Y-m-d').' 15:59:59.999';
        }else{
            $start = date('Y-m-d',strtotime('-1 day')).' 16:00:00.000';
            $end = date('Y-m-d').' 03:59:59.999'; 
        }
        $querySql = "select * from salesorder where payamount > 1000 and quantity > 20 and recordDate between '{$start}' and '{$end}'";
