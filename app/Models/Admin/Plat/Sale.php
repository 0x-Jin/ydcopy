...
class Sale extends ModelWithoutMysql {
    ...
    protected  function _genSql($params){
        ...
        $aIsObSoleteWhere = empty($params['isObSolete']) ? 'and a.isObsolete = 0' : "";
        $bIsObSoleteWhere = empty($params['isObSolete']) ? 'and b.isObsolete = 0' : "";
        $dIsObSoleteWhere = empty($params['isObSolete']) ? 'and d.isObsolete = 0' : "";
...
(
    ...
    protected function _fmtData($rawData){
        ...
        foreach($rawData as &$row){
            $row['商品金额'] = (float)$row['商品金额'];
            $row['折后商品金额'] = (float)$row['折后商品金额'];
            $row['商品折让金额'] = (float)$row['商品折让金额'];
            $row['支付金额'] = (float)$row['支付金额'];
            $row['邮费'] = (float)$row['邮费'];
            $row['商品数量'] = (float)$row['商品数量'];
            $row['折让金额'] = (float)$row['折让金额'];
            $row['明细商品总数'] = (float)$row['明细商品总数'];
