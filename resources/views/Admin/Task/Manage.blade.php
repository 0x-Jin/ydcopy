...
<script>
    ...
    function show_task_progress(task_id){
        $.ajax({
    ...
    function fastedit(id,obj){
       ...
       $(obj).prev().remove(); 
       var input =  $("<input type='text' name='rankids[]' value='"+num+"' style='width: 20%;' />");
       $(obj).before(input);
       input.focus();
       input.blur(function(){
          var after = input.val();
        ...
        function dialog(task_id,obj) {
            ...
            $.ajax({
                ...
                success:function(data){
                    var str = '';
                    for(var i in data){
                        str += "<div id='popOverBox' style='margin-left: 21%;'>"+data[i].current_depart+"—"+data[i].current+"—"+data[i].status+"</div>"
                    }
                    $(obj).popover({
                        trigger:'manual',
                        placement : 'bottom',
                        title :'<div style="text-align:center;font-size:14px;" id="titletest">完成状况</div>',
                        html: 'true',
                        content :"<div  class='dailog'>"+str+"</div>",
                        animation: false,
                     show: 0, hide: 100 ,
                    delay:0
                    }).mousemove(function(){
