...
class ManageController extends OmsController {
        ...
        if ($detail_ids) {
            ...
            if (!empty($data)) {
                foreach ($data as $item) {
                    ...
                    if (empty($pinfo)) {
                        $item->currentone = "未知人";
                        $item->current_department =" <a rel='drevil'  onmouseover='test(" . $item->task_id . ",this)'>". "未知部门" . "<span class='fa fa-search glyphicon glyphicon-shopping-cart'></span></a>";
                    } else {
                        $item->currentone = $pinfo[0]->Code;
                        $item->current_department = "<a  rel='drevil'  onmouseover='test(" . $item->task_id . ",this)'>" .$pinfo[0]->Name . "<span class='fa fa-search glyphicon glyphicon-shopping-cart'></span></a>";
                    }
    ...
    public function edit(Request $req) {
        ...
        if ($this->is_admin()) {
            ...
            foreach ($chainData as $k => $v) {
                ...
                $chainData[$k]['currentone'] = $apiinfo[0]->Code;
            ...
            foreach ($chainData as $k => $v) {//判断链条
                if ($v['currentone'] === $currentuserid) {//说明是可以看到的;
                    if ($v['order'] == 1) {//如果是管理人发起的，如何解决
                        ...
                        $next['data']['currentone'] = $apiinfo[0]->Code;
...
//                            $chainData[$k]['addons'] = [];
                        ...
                        $current['data']['currentone'] = $apiinfo[0]->Code;
                        ...
                        foreach ($chainData as $kk => $vv) {
                            ...
                            if ($vv['order'] < $v['order']) {
                                ...
                                $last['data']['currentone'] = $apiinfo[0]->Code;
        ...
        if ($flag == 2) {
            return view('Admin/Task/editManage', ['current' => $current, 'mainData' => $maindata, 'next' => $next, 'last' => $alllast, 'alldepartment' => $alldepartment]);
        } elseif ($flag == 1) {//说明是第一个人
            $current['data']['title'] = $maindata['title'];
            return view('Admin/Task/begineditManage', ['current' => $current, 'mainData' => $maindata, 'next' => $next, 'last' => $last]);
        } else {
    ...
    public function doedit(Request $req) {
        ...
        if (!empty($req->input('finished'))) {//完结没有进行验证
            ...
            if (DB::table('yfdyf_task')->where('task_id', $node_id->task_id)->update(['status' => 'finished'])) {
                return view('Admin/Common/Message', ['title' => '操作提示', 'url' => $this->return_url, 'message' => '当前任务完结成功']);
            }
            return;
        }
        if (!empty($req->input('save'))) {//保存验证
//            if(empty($current['expire_time'])){
//                
//            }
            if (!empty($current['addons'])) {
                $current['addons'] = implode(',', $current['addons']);
            ...
            $myself = array(
                ...
                'rank' => isset($current['rank']) ? intval($current['rank']) : '',
                'remark' => isset($current['remark']) ? $current['remark'] : '',
                'expire_time' => isset($current['expire_time']) ? strtotime($current['expire_time']) : '',
        ...
        if (!empty($req->input('turn'))) {//增加重复判断
            ...
            if ($currentperson == $next['currentone']) {
                return view('Admin/Common/Message', ['title' => '操作提示', 'url' => $this->return_url, 'message' => '上下级人员重复']);
            }
            ...
            $myself = array(
                'remark' => isset($current['remark']) ? $current['remark'] : '',
                'addons' => $current['addons'],
                'rank' => isset($current['remark']) ? intval($current['rank']) : '',
                'edittime' => Carbon::now()->timestamp,
                'expire_time' => isset($current['expire_time']) ? strtotime($current['expire_time']) : '',
                'op_time' => empty($current['op_time']) ? Carbon::now()->timestamp : strtotime($current['op_time']),
                'status' => 'finished',
                'nextone' => $next['currentone']
            );
            if (DB::table('yfdyf_task_detail')->where('detail_id', $detail_task_id)->update($myself)) {
                $newdata = array(
    ...
    public function downFile(Request $req) {
        ...
        if (code == sha1(base64_encode($task_id . '-' . $detail_id . '-' . $list) . base64_encode($this->salt))) {
            if (file_exists($file)) {
                header('Content-Code: File Transfer');
    ...
    public function doadd(Request $req) {
        ...
        if ($myinfo->User_id == $reqdata['getone']) {
            return view('Admin/Common/Message', ['title' => '操作提示', 'url' => $this->return_url, 'message' => '添加人和接收人不能是同一个人']);
        }
    ...
    public function showProgress(Request $req) {
        ...
        $json_array = [];
         foreach ($chainData as $v) {
                $apiinfo = $this->getDepartmentByPid($v['currentone'], 'User_id');
                $data['current'] = $apiinfo[0]->Code;
                $data['current_depart'] = $apiinfo[0]->Name;
                if (empty($v['current_department'])) {
                    $data['current_depart'] = "未知部门";
                }
                $data['status'] = $this->to_map['status'][$v['status']];
                $json_array[] = $data;
        }
...
//        echo $table;
    ...
    public function viewTask(Request $req) {
        ...
        $mainData = TaskManage::find($task_id)->get()->toArray();
        $mainData = $mainData[0];
        $chainData = TaskManage::find($task_id)->TaskDetail()->orderBy('order', 'asc')->get()->toArray();
        $newData = array();
        foreach ($chainData as $v) {
        ...
        return view('Admin/Task/viewManage', ['newData' => $newData,'mainData'=>$mainData]);
    }
    ...
    public function doAdmin(Request $req) {
        ...
        foreach ($allData['current']["'currentone'"] as $k => $v) {
            ...
            $ids[] = $k;
            if ($i > 1) {
                if ($v == $allData['current']["'currentone'"][$ids[$i-2]]) {
                    $canAdd = false;
                }
            }
        }
        ...
        if ($canAdd) {
            ...
            foreach ($ids as $k => $v) {
                ...
                DB::table('yfdyf_task_detail')->where('detail_id', $v)->update($slave_data); //把当前级结束：
            }

            if (!empty($req->input('turn'))) {//流转
                if (empty($allData['next']['currentone']) || empty($allData['next']['current_department'])) {
                    return view('Admin/Common/Message', ['title' => '操作提示', 'url' => $this->return_url, 'message' => '部门和人不能为空']);
                }
                //找到最后一个人：
                $ntdata = DB::table('yfdyf_task_detail')->where('task_id', $task_id)->orderBy('order', 'desc')->limit(1)->get();
                $ntdata = $ntdata[0];
                $updatedata = array(
                    'nextone' => $allData['next']['currentone'],
                    'status' => 'finished',
                    'current_department' => $allData['next']['current_department']
                );
                $insertdata = array(
                    'order' => $ntdata->order + 1,
                    'lastone' => $ntdata->currentone,
                    'status' => 'on',
                    'task_id' => $ntdata->task_id,
                    'currentone' => $allData['next']['currentone'],
                    'current_department' => $allData['next']['current_department'],
                    'op_time' => isset($allData['next']['op_time']) ? strtotime($allData['next']['op_time']) : null,
                    'expire_time' => isset($allData['next']['expired_time']) ? strtotime($allData['next']['expired_time']) : null,
                    'remark' => $allData['next']['remark'],
                    'rank' => intval($allData['next']['rank'])
                );
                if ($ntdata->currentone == $allData['next']['currentone']) {
                    return view('Admin/Common/Message', ['title' => '操作提示', 'url' => $this->return_url, 'message' => '不可以流转给同一人']);
                }
                DB::table('yfdyf_task_detail')->where('detail_id', $ntdata->detail_id)->update($updatedata);
                DB::table('yfdyf_task_detail')->insert($insertdata);
            }
            if (!empty($req->input('finished'))) {//
                DB::table('yfdyf_task')->where('task_id', $task_id)->update(
                        array('status' => 'finished'));
                DB::table('yfdyf_task_detail')->where('task_id', $task_id)->update(
                        array('status' => 'finished')
                );
            }
            return view('Admin/Common/Message', ['title' => '操作提示', 'url' => $this->return_url, 'message' => '当前任务更新成功']);
        } else {
            return view('Admin/Common/Message', ['title' => '操作提示', 'url' => $this->return_url, 'message' => '当前链中存在上下级人员重复']);
        }
    }
