...
//            $row['分类4'] = trim($row['分类4']) == '000' ? '' : $row['分类4'];
            $row['销售数量'] = (float)$row['销售数量'];
            $row['销售金额'] = (float)$row['销售金额'];
            $row['实际销售总金额'] = (float)$row['实际销售总金额'];
            $row['均价成本总额'] = (float)$row['均价成本总额'];
            $row['总毛利额'] = (float)$row['总毛利额'];
            $row['实际毛利额'] = (float)$row['实际毛利额'];
        }
        unset($row);
        return $rawData;
    }
