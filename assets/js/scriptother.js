var App = function () {
	...
	var handleRaty = function () {
		$.fn.raty.defaults.path = 'static/js/jquery-raty/img';
		...
		$('#starHalf-demo').raty({
			path    : 'static/js/jquery-raty/img',
			half    : true,
	...
	var handleTimeline = function () {
		createStoryJS({
			...
			width:		'100%',
			height:		'600',
			source:		'static/js/timelinejs/example_json.json',
			embed_id:	'my-timeline',
			debug:		true,
			css:        'static/js/timelinejs/css/timeline.css',
            js:         'static/js/timelinejs/js/timeline-min.js'
		});
	}
	/*-----------------------------------------------------------------------------------*/
	/*	Handle Slidernav
		...
		if ($.cookie('gritter_show')) {
            ...
            setTimeout(function () {
                ...
                var unique_id = $.gritter.add({
                    ...
                    // (string | optional) the image to display on the left
                    image: 'static/img/gritter/buy.png',
                    // (bool | optional) if you want it to fade out on its own or just sit there
                    sticky: true,
                    // (int | optional) the time you want it to be alive for before fading out
                    time: '',
	...
	var handleProfileEdit = function () {
		if($('.datepicker')){
            $(".datepicker").bind("focusin",function(){
                $(this).datepicker({
                    changeMonth: true,
                    changeYear: true,
                    dateFormat: "yy-mm-dd"
	...
	return {
        ...
        setPage: function (name) {
            currentPage = name;//dynamic_table
        },
        ...
        blockUI: function (el, loaderOnTop) {
            lastBlockedUI = el;
            jQuery(el).block({
                message: '<img src="/admin/assets/img/loaders/12.gif" align="absmiddle">',
                css: {
                    border: 'none',
                    padding: '2px',
